...
def main(argv):
    ...
    mergednodes = ''
    deletednodes = ''
    output_ranks = ['superkingdom', 'kingdom', 'phylum',
                    'class', 'order', 'family', 'genus', 'species']
    col = 3
    read = 2
    try:
        opts, args = getopt.getopt(argv, "hd:m:o:i:e:l:c:r:", [
                                   "nodes=", "names=", "out=", "input=", "merged=", "deleted=", "column=", "read="])
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            read = int(arg)
    ncbi_full_taxonomy = NcbiTaxonomyFromFiles(
        open(nodes_file), open(names_file))
    merged = {}
        ...
        if not curr_taxid in node_numbers:
            try:
                ...
                try:
                    ...
                    print(
                        'The following taxid could not be added to included_nodes: taxid=' + str(curr_taxid) + '.\n')
